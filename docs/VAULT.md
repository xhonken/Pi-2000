...
Create your Vault with two different passphrases of at least 14 characters.
...
- An open entry or private operation locks after **30 seconds of inactivity**.
  Typing and clicking inside Vault renew this period. The list locks after five
  minutes of inactivity. Hiding the browser tab, minimising Vault, closing Vault
...
- Save edits before leaving. Closing or automatic locking discards unsaved edits;
  Vault deliberately does not send plaintext drafts to other editor/storage apps.
...
File → Export Encrypted downloads the complete encrypted Vault after B unlock.
...
current account's Vault. Export This Entry inside an unlocked entry is a separate,
...
If you missed saving it, use your current A and B to change the Vault passwords
and generate a new recovery key.

Security → Recovery accepts your recovery key and new A/B passphrases. An
administrator cannot reset these Vault passwords to read old data. If you lose
...
The normal application SQLite backup includes the encrypted Vault records. An
account's storage quota includes its ciphertext. Deleting a Pi-2000 account deletes
its live Vault row through a foreign key; existing backups/exports retain copies.
A Pi-2000 login password reset does not change the Vault's encryption passwords.
...
its saved Vault has changed elsewhere. A local logout/account-change event clears
it immediately; a BroadcastChannel also closes Vault windows in other tabs when
...
binding the format, random Vault ID and purpose/entry ID. Password derivation runs
...
when unlocked. Use the Vault in your own trusted browser, not the Pi-hosted streamed
