...
Pi-Arduino is a native Pi-2000 application for writing sketches, installing board support and libraries, compiling on the Pi, uploading over the Pi's USB connection and using a serial monitor. Open the **Pi-Arduino** desktop icon or **Start → Programs → Development and Drawing → Pi-Arduino**.

Pi-Arduino is included in Alpha 5 source and the arm64 test package.
...
3. Open **Tools → Boards Manager** and wait for the platform list. Missing catalogs and builtin tools are downloaded automatically. Choose **ESP32 by Espressif Systems**, select a version and click **Install Version**. The first ESP32 installation downloads several GB and can take several minutes. Arduino AVR boards are also available.
...
5. Add libraries using **Tools → Library Manager**. Missing catalogs are downloaded automatically on the first search; no board installation is required to search. Use **Refresh Indexes** to update the catalogs later. Search by function or name, read the description and supported architectures, choose a version and click **Install Library**. The official CLI installs required library dependencies automatically.
