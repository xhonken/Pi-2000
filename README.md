...
Alpha 6 includes guarded recovery and encrypted backup export,
...
[one-command test setup](docs/TESTING.md). Hardware and fresh-install acceptance remain separate checks.
...
SSH jobs run in the separate `pi2000-sessions` service. Disconnecting your computer, reloading the page or logging off disconnects the display; the job can continue on the server. Logging in again restores the saved workspace and reconnects to available sessions. Up to eight terminal sessions per account are supported, including ended sessions retaining output.
...
The nightly backup timer creates verified local snapshots containing the SQLite database, file blobs and browser profiles. Seven backups are retained under `/var/backups/pi2000`. Browser processes are briefly frozen for a consistent filesystem snapshot. Restores are staged and validated before replacing data. Backups on the same disk do not protect against disk failure; an off-device backup plan remains necessary.
