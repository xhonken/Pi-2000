...
A Win UI container is a semantic `fieldset` with a visible `legend`. Use `win2k-group` for the shared Windows surface and `win2k-reference-group` for standard page spacing:
...
The distributable framework lives in the top-level `win-ui/` package. Its `core/` directory contains platform-independent CSS and JavaScript and never imports WordPress.

- `win-ui/standalone/` is the plain HTML starter.
- `win-ui/integrations/wordpress/theme/` is the optional block-theme adapter.
- `win-ui/integrations/wordpress/plugin/` contains optional portable content types.
- `win-ui/integrations/wordpress/headless/` is the optional read-only REST adapter.
