(function bootstrapWinUI(global) {
  'use strict';
  ...
  const getConfig = (root, overrides = {}) => {
    const host = root.matches?.('[data-winui-app],.winui-app') ? root : root.querySelector?.('[data-winui-app],.winui-app');
  ...
  const init = (root = document, overrides = {}) => {
    const app = root.matches?.('[data-winui-app],.winui-app') ? root : root.querySelector?.('[data-winui-app],.winui-app');
    if (!app) return null;
    ...
    const startButton = app.querySelector('[data-winui-start-button]');
    const startMenu = app.querySelector('[data-winui-start-menu]');
    const statusFields = [...app.querySelectorAll('[data-winui-status]')];

    const closeCascades = (except) => {
      app.querySelectorAll('[data-winui-cascade]').forEach((trigger) => {
    ...
    const menuDetails = [...app.querySelectorAll('details[data-winui-menu]')];
    ...
    on(document, 'click', (event) => {
      ...
      if (!event.target.closest('[data-winui-start-menu],[data-winui-start-button]')) closeStart();
      if (!event.target.closest('details[data-winui-menu]')) menuDetails.forEach((menu) => { menu.open = false; });
    });
    ...
    on(document, 'keydown', (event) => {
      ...
      if (event.key === 'F1' && config.help) {
        event.preventDefault();
        const helpLink = app.querySelector('[data-winui-help-link]');
        global.location.assign(helpLink?.href || config.helpUrl);
      }
    });

    app.querySelectorAll('[data-winui-help]').forEach((element) => {
      element.hidden = !config.help;
    });

    const updateClock = () => {
      const now = new Date();
      app.querySelectorAll('[data-winui-clock]').forEach((element) => {
        element.textContent = new Intl.DateTimeFormat(config.locale, { hour: '2-digit', minute: '2-digit' }).format(now);
      });
      app.querySelectorAll('[data-winui-date]').forEach((element) => {
    ...
    const notify = (message, timeout = 2400) => {
      ...
      global.clearTimeout(app.__winuiStatusTimer);
      app.__winuiStatusTimer = global.setTimeout(() => {
        statusFields.forEach((field) => { field.textContent = config.readyText; });
      }, timeout);
    };

    on(app, 'winui:status', (event) => notify(event.detail?.message || config.readyText, event.detail?.timeout));
    ...
    app.dataset.winuiReady = 'true';
    app.dispatchEvent(new CustomEvent('winui:ready', { detail: api }));
    return api;
  };

  global.WinUI = Object.freeze({ init, defaults });
