...
async def delete_user(request):
    ...
    await asyncio.to_thread(shutil.rmtree, STATE/'git-workspaces'/str(user_id), True)
    if WORKER_SOCKET:
...
def make_app():
    ...
    phpmyadmin=PhpMyAdmin(sys.modules[__name__],databases)
    app.cleanup_ctx.append(phpmyadmin.lifecycle)
    app.router.add_post('/api/phpmyadmin/launch',phpmyadmin.launch)
    app.router.add_delete('/api/phpmyadmin/session/{sid}',phpmyadmin.disconnect)
    ...
    app.router.add_delete('/api/development/requests/{id}',api_client.collections)
    app.router.add_post('/api/development/http',api_client.send)
