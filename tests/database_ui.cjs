...
(async()=>{
 ...
 try{
  ...
  if(process.env.WIN2K_TEST_DB_PORT){
   ...
   await w.locator('.db-messages').filter({hasText:'Row updated'}).waitFor();
  ...
  if(process.env.WIN2K_TEST_DB_PORT){
   ...
   await menu('Data','Insert Row');let form=page.locator('dialog.db-dialog').last();
   ...
   await page.waitForFunction(()=>document.querySelector('.qb-dialog .db-admin-preview').textContent.includes('LEFT JOIN'));
   await form.getByRole('tab',{name:'Columns and Filters'}).click();
   await form.locator('[name=columns]').selectOption(['t1.title','t2.name']);
   await form.getByRole('tab',{name:'Tables and Joins'}).click();
   await form.locator('[data-kind]').selectOption('RIGHT');
   ...
   await page.screenshot({path:'/tmp/pi2000-query-builder.png'});
   await form.getByRole('button',{name:'Open SQL Query',exact:true}).click();
   await w.getByRole('button',{name:'Execute',exact:true}).click();
   await w.locator('.db-results').filter({hasText:'Ada'}).waitFor();
   assert((await w.locator('.db-results').textContent()).includes('Saved directly'));
   await w.locator('[data-object=z_notes]').click();
   await w.getByRole('button',{name:'Select row 1',exact:true}).click();
   await menu('Data','Delete Row');
   await page.locator('dialog.db-dialog').last().getByRole('button',{name:'OK',exact:true}).click();
