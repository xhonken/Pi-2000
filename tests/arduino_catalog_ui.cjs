...
// its empty disposable runtime, never PI2000_TEST_ARDUINO_RUNTIME.
const {chromium}=require('playwright');
const assert=require('node:assert/strict');
(async()=>{
 assert.ok(!process.env.PI2000_TEST_ARDUINO_RUNTIME,'This regression needs an empty runtime');
 ...
 try {
  ...
  await page.screenshot({path:require('node:path').join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','pi2000-arduino-catalog-cold.png')});
