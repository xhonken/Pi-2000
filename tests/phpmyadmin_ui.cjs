...
(async()=>{const browser=await chromium.launch({executablePath:process.env.PI2000_TEST_CHROMIUM||'/usr/bin/chromium',headless:true});try{
const origin=process.env.PI_TEST_ORIGIN||(process.env.PI2000_TEST_URL||'http://127.0.0.1:18765');const page=await browser.newPage({ignoreHTTPSErrors:!!process.env.PI_TEST_ORIGIN,viewport:{width:1280,height:900}});const errors=[];page.on('pageerror',e=>errors.push(e.message));
...
const port=process.env.PI2000_TEST_DB_PORT;
await page.evaluate(async port=>{const r=await fetch('/api/databases/connections',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:'phpMyAdmin fixture',host:'127.0.0.1',port:Number(port),username:'root',password:'',database:'',tls:'disabled',ca:'',save_password:true})});if(!r.ok)throw Error(await r.text());Pi2000Shell.actions.database();},port);
...
await f.locator('#topmenucontainer').waitFor({timeout:30000});await noNotices();await page.screenshot({path:require('node:path').join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','pi2000-phpmyadmin.png')});

assert(await f.locator('link[href="./pi2000.css"]').count());
await f.locator('#topmenu a[href*="route=/server/sql"]').click();
await f.locator('#sqlqueryform .CodeMirror').waitFor({timeout:10000}).catch(async e=>{console.log('SQL PAGE',await f.locator('body').innerText());await noNotices();await page.screenshot({path:require('node:path').join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','pi2000-pma-failure.png')});throw e;});
...
await noNotices();await page.screenshot({path:require('node:path').join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','pi2000-pma-rows.png')});
...
await noNotices();await page.screenshot({path:require('node:path').join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','pi2000-pma-small.png')});
