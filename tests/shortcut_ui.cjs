...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>Pi2000Shell.actions.add());
  ...
  await icon.dblclick();await page.locator('.browser-window iframe').waitFor();
  ...
  await page.evaluate(()=>Pi2000Shell.actions.trash());
  ...
  await icon.focus();page.once('dialog',d=>d.accept());await page.keyboard.press('Delete');await icon.waitFor({state:'detached'});
