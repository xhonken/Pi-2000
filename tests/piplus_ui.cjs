...
(async()=>{
 ...
 try{
  ...
  await page.goto(installed?.origin||(process.env.PI2000_TEST_URL||'http://127.0.0.1:18765'));if(!installed){await page.locator('#login-form [name=password]').fill('browser-test-password');await page.locator('#login-form [type=submit]').click();}await page.locator('#session').waitFor({state:'visible'});
  ...
  await w.getByRole('tab',{name:'display.cpp',exact:true}).waitFor();await page.evaluate(id=>Pi2000Editor.openFile(id),files[1].id);await w.getByRole('tab',{name:'config.h',exact:true}).waitFor();
  ...
  await page.evaluate(()=>{for(const t of Pi2000Editor.remoteContext().tabs)t.session.insert({row:0,column:0},'// display test\n');});
  ...
  await page.keyboard.press('Control+F2');await page.evaluate(()=>{const c=Pi2000Editor.remoteContext();c.active.session.insert({row:0,column:0},'// added above bookmark\n');ace.edit(document.querySelector('.editor-code')).gotoLine(1);});await page.keyboard.press('F2');
  ...
  await command(page,w,'Language','Select Language…');await form.locator('[name=filter]').fill('Python');await form.locator('[name=language]').selectOption('ace/mode/python');await form.getByRole('button',{name:'Use Language',exact:true}).click();await page.waitForFunction(()=>Pi2000Editor.remoteContext().active.session.getMode().$id==='ace/mode/python');
  await w.getByRole('menuitem',{name:'Encoding',exact:true}).click();await page.getByRole('menuitemcheckbox',{name:'Convert to Windows (CR LF)',exact:true}).click();await w.getByRole('button',{name:'Save',exact:true}).click();await page.waitForFunction(()=>Pi2000Editor.remoteContext().active.saved.includes('\r\n'));
  ...
  await w.getByRole('tab',{name:/display.cpp/}).click();await w.getByRole('button',{name:'Save',exact:true}).click();await page.waitForFunction(()=>Pi2000Editor.remoteContext().tabs.every(t=>t.saved===t.session.getValue()));
  assert.equal(Math.round((await w.locator('.editor-toolbar .pi2000-pixel-icon').first().boundingBox()).width),18);await page.screenshot({path:require('node:path').join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','pi2000-piplus-desktop.png')});
  await page.reload();await w.getByRole('tab',{name:'display.cpp',exact:true}).waitFor();assert.equal(await page.evaluate(()=>ace.edit(document.querySelector('.editor-code')).getShowInvisibles()),true);assert.equal(await page.evaluate(()=>Pi2000Editor.remoteContext().active.session.getTabSize()),2);
  // A cancelled Save As must stop Save All before later unsaved documents.
  await page.evaluate(()=>{const c=Pi2000Editor.remoteContext();c.addTab('one.txt','one').saved=null;c.addTab('two.txt','two').saved=null;});await w.getByRole('button',{name:'Save All',exact:true}).click();await page.locator('#editor-save-form').waitFor();assert.equal(await page.locator('#editor-save-form [name=name]').inputValue(),'one.txt');await page.locator('#close-window').click();await page.waitForTimeout(200);assert.equal(await page.locator('#window').evaluate(e=>e.open),false);
  // Import preserves UTF-8 BOM and CR LF; exporting uses the exact text.
  await command(page,w,'File','Import from Computer…');await form.locator('[name=files]').setInputFiles({name:'utf8.txt',mimeType:'text/plain',buffer:Buffer.from('\ufeffRäksmörgås\r\n')});await form.getByRole('button',{name:'Open Files',exact:true}).click();await w.getByRole('tab',{name:'● utf8.txt',exact:true}).waitFor();assert.equal(await page.evaluate(()=>Pi2000Editor.remoteContext().active.session.getValue()),'\ufeffRäksmörgås\r\n');
  ...
  await page.screenshot({path:require('node:path').join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','pi2000-piplus-narrow.png')});const box=await w.locator('.editor-code').boundingBox();assert.ok(box.width>250&&box.height>150);assert.deepEqual(errors,[]);
