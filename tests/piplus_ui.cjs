...
(async()=>{
 ...
 try{
  ...
  await command(page,w,'Language','Select Language…');await form.locator('[name=filter]').fill('Python');await form.locator('[name=language]').selectOption('ace/mode/python');await form.getByRole('button',{name:'Use Language',exact:true}).click();assert.equal(await page.evaluate(()=>Win2kEditor.remoteContext().active.session.getMode().$id),'ace/mode/python');
  ...
  assert.equal(Math.round((await w.locator('.editor-toolbar .win2k-pixel-icon').first().boundingBox()).width),18);await page.screenshot({path:'/tmp/pi2000-piplus-desktop.png'});
  ...
  await page.screenshot({path:'/tmp/pi2000-piplus-narrow.png'});const box=await w.locator('.editor-code').boundingBox();assert.ok(box.width>250&&box.height>150);assert.deepEqual(errors,[]);
