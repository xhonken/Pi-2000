...
class BackupTests(unittest.IsolatedAsyncioTestCase):
    async def test_snapshot_restores_database_profiles_and_omits_login_tokens(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            code=root/'code';(code/'phpmyadmin').mkdir(parents=True)
            (code/'phpmyadmin/config.inc.php').write_text('<?php /* gateway config */')
            (code/'nested/deeper').mkdir(parents=True)
            (code/'nested/deeper/__init__.py').write_text('VALUE = 42\n')
            (code/'PROJECT-HANDOVER.local.md').write_text('private fixture')
            (code/'build-info.json').write_text('{"version":"fixture"}')
            archive=await snapshot(state,root/'backup','',code)
            dest=root/'restored';restore(archive,dest)
            self.assertTrue((dest/'code/phpmyadmin/config.inc.php').is_file())
            self.assertEqual((dest/'code/nested/deeper/__init__.py').read_text(),'VALUE = 42\n')
            self.assertFalse((dest/'code/PROJECT-HANDOVER.local.md').exists())
            self.assertEqual(json.loads((dest/'code/build-info.json').read_text())['version'],'fixture')
