...
(async()=>{
 ...
 const state=p=>p.evaluate(()=>Pi2000Desktop.api('/workspace'));
 try{
  await start();browser=await chromium.launch({executablePath:process.env.PI2000_TEST_CHROMIUM||'/usr/bin/chromium',headless:true});
  const first=await login(0);for(let i=1;i<4;i++)await first.evaluate(async i=>Pi2000Desktop.api('/users','POST',{username:'worker'+i,password:'recovery-test-password'}),i);
  const pages=[first];for(let i=1;i<4;i++)pages.push(await login(i));
  for(let i=0;i<4;i++){
   const p=pages[i];await p.evaluate(i=>{Pi2000Shell.actions.calculator();document.querySelector('.calculator-form input').value=String(i)+'+42';Pi2000Shell.actions.editor();ace.edit(document.querySelector('.editor-code')).setValue('unsaved account '+i,-1);Pi2000Shell.actions.notes();},i);
   await p.locator('.notes-text:not([disabled])').fill('notes account '+i);
   await until(()=>p.evaluate(async()=>{const d=await Pi2000Desktop.api('/documents');return d.documents.some(x=>x.key.startsWith('draft-'))&&d.documents.some(x=>x.key==='notes');}));
   await p.evaluate(async i=>{const c=Pi2000Desktop.listWindows().find(w=>w.type==='calculator-window');c.element.style.left=(20+i*30)+'px';c.element.style.top='55px';c.element.style.width='420px';c.element.style.height='410px';c.element.querySelector('[data-control=min]').click();if(i===2)Pi2000Desktop.listWindows().find(w=>w.type==='editor-window').element.classList.add('maximized');await Pi2000Desktop.saveWorkspace();},i);
   await until(()=>p.evaluate(async i=>(await Pi2000Desktop.api('/workspace')).windows.find(w=>w.type==='calculator-window')?.state.expression===String(i)+'+42',i));
  }
  // Additional working documents include unsaved source and drawing edits.
  await first.evaluate(async()=>{
   Pi2000Shell.actions.arduino();const w=Pi2000Desktop.listWindows().find(w=>w.type==='arduino-window');await w.ready;
   const r=await fetch('/api/development/arduino',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'create',name:'RecoverySketch'})});if(!r.ok)throw Error(await r.text());const created=await r.json();const project=await Pi2000Desktop.api('/development/arduino','POST',{action:'open',project:created.id});await w.restoreState({project});ace.edit(document.querySelector('.arduino-editor')).setValue('// unsaved reboot sketch\n',-1);
   Pi2000Shell.actions.cad();await Pi2000Desktop.listWindows().find(w=>w.type==='cad-window').ready;
   Pi2000Shell.actions.displaystudio();Pi2000Shell.actions.vault();
  });
  await first.locator('.cad-window input[name=outerwidth]').fill('444');
  await first.evaluate(()=>{const w=Pi2000Desktop.listWindows().find(w=>w.type==='display-window'),s=w.captureState();s.doc.layers.push({type:'text',text:'Recovered drawing',x:5,y:5,w:20,h:20,fontSize:14,lineWidth:1,fill:'#ffffff',stroke:'#ffffff',font:'Arial',filled:true,points:[],image:''});s.dirty=true;return w.restoreState(s);});
  await until(()=>first.evaluate(async()=>{await Pi2000Desktop.saveWorkspace();return !!(await Pi2000Desktop.api('/workspace')).windows.find(w=>w.type==='arduino-window')?.state.project.files['RecoverySketch.ino']?.includes('unsaved reboot sketch');}));
  // Represent a terminal whose OS process will no longer exist after reboot.
  await first.evaluate(async()=>{const profile=await Pi2000Desktop.api('/items','POST',{kind:'profile',name:'Recovery SSH',host:'127.0.0.1',port:22,username:'fixture'});const saved=await Pi2000Desktop.api('/workspace');saved.windows.push({type:'terminal-window',profile:profile.id,terminal:'previous-process',left:50,top:80,width:650,height:430,hidden:false,maximized:false});await Pi2000Desktop.api('/workspace','PUT',saved);});
  ...
  for(let i=0;i<4;i++){
   ...
   try{await p.waitForFunction(i=>ace.edit(document.querySelector('.editor-code')).getValue()==='unsaved account '+i,i);}catch(e){console.log('recovery diagnostic',i,await p.evaluate(async()=>({windows:Pi2000Desktop.listWindows().map(w=>({type:w.type,status:w.status.textContent})),text:document.querySelector('.editor-code')?ace.edit(document.querySelector('.editor-code')).getValue():null,docs:await Pi2000Desktop.api('/documents'),workspace:await Pi2000Desktop.api('/workspace')})));throw e;}
  ...
  assert.equal(await p.evaluate(()=>Pi2000Desktop.listWindows().find(w=>w.type==='display-window').captureState().doc.layers[0].text),'Recovered drawing');
  ...
  await q.unroute('**/api/workspace');await until(()=>q.evaluate(async()=>(await Pi2000Desktop.api('/workspace')).windows.find(w=>w.type==='calculator-window').state.expression==='offline recovery'));
  ...
  await q.locator('.calculator-form input').fill('first tab wins');await until(()=>q.evaluate(async()=>(await Pi2000Desktop.api('/workspace')).windows.find(w=>w.type==='calculator-window').state.expression==='first tab wins'));
  ...
  await restored[2].evaluate(data=>Pi2000Desktop.api('/workspace','PUT',data),broken);
  const recovered=await login(2);assert.equal(await recovered.locator('.editor-window').count(),1);assert.match(await recovered.locator('.display-window .app-status').innerText(),/Recovery could not finish/);await recovered.evaluate(()=>Pi2000Desktop.saveWorkspace());assert.equal((await state(recovered)).windows.find(w=>w.type==='display-window').state.doc.format,'broken');
  ...
  await restored[3].locator('.calculator-form input').fill('before slow save');const initial=restored[3].evaluate(()=>Pi2000Desktop.saveWorkspace());await until(()=>intercepted);
  await restored[3].locator('.calculator-form input').fill('after slow save');const final=restored[3].evaluate(()=>Pi2000Desktop.saveWorkspace());release();await initial;await final;assert.equal((await state(restored[3])).windows.find(w=>w.type==='calculator-window').state.expression,'after slow save');await restored[3].unroute('**/api/workspace');
  await p.screenshot({path:path.join(process.env.PI2000_TEST_ARTIFACTS||temp,'workspace-restored.png')});
