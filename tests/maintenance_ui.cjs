...
(async()=>{
 ...
 try{
  ...
  await page.evaluate(()=>Pi2000Shell.actions.about());
  ...
  await page.keyboard.press('Escape');await page.evaluate(()=>Pi2000Shell.actions.status());
  ...
  await page.screenshot({path:path.join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','maintenance-status.png')});
