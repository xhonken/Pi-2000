...
(async()=>{const browser=await chromium.launch({executablePath:process.env.PI2000_TEST_CHROMIUM||'/usr/bin/chromium',headless:true});
 ...
 await page.locator('#desktop-icons [data-action=arduino]').dblclick();const w=page.locator('.arduino-window');await w.waitFor();
 ...
 const real=!!process.env.PI2000_TEST_ARDUINO_RUNTIME;
 if(real){
  ...
  await page.screenshot({path:require('node:path').join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','pi2000-arduino-library-manager.png')});
 ...
 await page.screenshot({path:require('node:path').join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','pi2000-arduino-workshop.png')});
 await w.locator('[data-control=close]').click();await page.evaluate(()=>Pi2000Shell.actions.arduino());await page.locator('.arduino-projects option').filter({hasText:'WorkshopUITest'}).waitFor();await page.locator('.arduino-projects').selectOption({label:'WorkshopUITest'});await page.getByRole('tab',{name:'helpers.h',exact:true}).waitFor();
 await page.setViewportSize({width:650,height:800});await page.evaluate(()=>document.documentElement.style.setProperty('--personal-font-size','18px'));await page.screenshot({path:require('node:path').join(process.env.PI2000_TEST_ARTIFACTS||'/tmp','pi2000-arduino-narrow.png')});
