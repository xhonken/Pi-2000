...
(async()=>{const browser=await chromium.launch({executablePath:'/usr/bin/chromium',headless:true});
 ...
 if(real){
  ...
  await page.screenshot({path:'/tmp/pi2000-arduino-library-manager.png'});
 ...
 await page.screenshot({path:'/tmp/pi2000-arduino-workshop.png'});
 ...
 await page.setViewportSize({width:650,height:800});await page.evaluate(()=>document.documentElement.style.setProperty('--personal-font-size','18px'));await page.screenshot({path:'/tmp/pi2000-arduino-narrow.png'});
