...
async function refreshUsers() {
 ...
 win.body.innerHTML = `<div class="explorer-toolbar"><button class="win2k-button" id="user-new">New User…</button><button class="win2k-button" id="user-refresh">Refresh</button><button class="win2k-button" id="user-self-password">Change My Password…</button></div><p class="users-help">Each user has their own devices and can connect via SSH. Administrators can manage users. Only the owner can change roles.</p><div class="users-list"><table><thead><tr><th>Username</th><th>Role</th><th>Status</th><th>Actions</th></tr></thead><tbody></tbody></table></div>`;
 for (const user of result.users) {
  const row = document.createElement('tr');
  row.innerHTML = `<td>${escape(user.username)}${user.id === account.id ? ' (you)' : ''}</td><td>${user.is_owner ? 'Owner' : user.role === 'admin' ? 'Administrator' : 'User'}</td><td>${user.active ? 'Enabled' : 'Disabled'}</td><td class="user-actions"></td>`;
...
function userConfirm(user, action) {
 ...
 shell.show(`${label} user`, `<p>Do you want to ${label.toLowerCase()} ${escape(user.username)}?</p><p>${action === 'delete' ? 'The account will be permanently deleted, including its folders, connections and desktop settings.' : user.active ? 'The user cannot log in until the account is enabled again.' : 'The user can log in again with their existing password.'}</p>${action === 'delete' || user.active ? '<p>Active logins and SSH terminals will be ended.</p>' : ''}<p class="error" id="user-error" role="alert"></p><div class="actions"><button class="win2k-button" data-action="close">Cancel</button><button class="win2k-button" id="user-confirm">${label}</button></div>`);
 $('#user-confirm').onclick = async e => {
  e.target.disabled = true;
  try { await api('/users/'+user.id, action === 'delete' ? 'DELETE' : 'PATCH', action === 'delete' ? undefined : {active:!user.active}); $('#window').close(); await refreshUsers(); }
  catch(error) { const el = $('#user-error'); if(el) el.textContent = error.message; }
  finally { e.target.disabled = false; }
 };
}
