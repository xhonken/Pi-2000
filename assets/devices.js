(() => {
'use strict';
const $ = s => document.querySelector(s), shell = window.Pi2000Shell;
...
const workspaceStatus=document.createElement('button');workspaceStatus.id='workspace-status';workspaceStatus.className='pi2000-button';
...
function locked() {
 workspaceGeneration++;workspaceReady=false;workspaceTag=null;workspaceSaved='';workspaceConflict=false;workspaceQueue=null;restoring=false;account = null; window.dispatchEvent(new CustomEvent('pi2000-user',{detail:null})); items = []; folder = null; selected = null; shell.setUser(null, api);
...
window.addEventListener('pi2000-session-expired',locked);
async function unlocked(user) { await shell.setUser(user, api); account = user; window.dispatchEvent(new CustomEvent('pi2000-user',{detail:user})); document.querySelectorAll('[data-action=users]').forEach(button => button.hidden = user.role !== 'admin'); $('#logon').hidden = true; $('#session').hidden = false; $('#login-form').reset(); $('#start-button').focus(); await restoreWorkspace(); }
...
async function refreshUsers() {
 ...
 win.body.innerHTML = `<div class="explorer-toolbar"><button class="pi2000-button" id="user-new">New User…</button><button class="pi2000-button" id="user-refresh">Refresh</button><button class="pi2000-button" id="user-self-password">Change My Password…</button></div><p class="users-help">Each user has a private Linux account and home. Administrators have Local Terminal, without automatic sudo. Only the creator can change roles.</p><div class="users-list"><table><thead><tr><th>Username</th><th>Role</th><th>Status</th><th>Actions</th></tr></thead><tbody></tbody></table></div>`;
 for (const user of result.users) {
  ...
  const button = (label, action) => { const b = document.createElement('button'); b.className = 'pi2000-button'; b.textContent = label; b.onclick = action; actions.append(b); };
...
async function restoreWorkspace() {
 ...
 try{
  ...
  for(const entry of saved.data.windows){
   if(account!==user||generation!==workspaceGeneration)return;
   try{
    const win=await window.Pi2000Apps.restore(entry,user,{remaining});
    if(account!==user||generation!==workspaceGeneration)return;
    if(win&&entry.state)await win.restoreState?.(entry.state);
    applyLayout(win,entry);
   }catch(e){
    if(account!==user||generation!==workspaceGeneration)return;
    const descriptor=Pi2000Apps.get(entry.type);if(!descriptor||!descriptor.allowed(user))continue;
...
shell.actions.browser=async(url)=>{
 ...
 win.body.innerHTML='<div class="browser-toolbar"><button class="pi2000-button browser-reconnect">Reconnect</button><button class="pi2000-button browser-fullscreen">Full Screen</button><button class="pi2000-button browser-stop">End Session…</button><span>Tabs and logins are private to your account.</span></div><div class="browser-content"><p class="browser-loading">Starting your browser…</p></div>';
...
function makeWindow(title, type) {
 const app=window.Pi2000Apps.get(type);
 ...
 element.innerHTML = `<header class="pi2000-titlebar"><strong>${escape(title)}</strong><div class="window-controls"><button class="pi2000-button" data-control="min" aria-label="Minimise">_</button><button class="pi2000-button" data-control="max" aria-label="Maximise">□</button><button class="pi2000-button" data-control="close" aria-label="Close">×</button></div></header><div class="app-body"></div><div class="app-status" role="status"></div>`;
 $('#session').append(element);
 const task = document.createElement('button'); task.className='pi2000-button task-entry';task.textContent=title;task.title=title;$('#tasks').append(task);
 ...
 window.Pi2000UI?.attach(win);return win;
}
...
function renderExplorer(){
 ...
 explorer.body.innerHTML=`<div class="explorer-toolbar"><button class="pi2000-button" id="folder-up" ${folder?'':'disabled'}>Up</button><button class="pi2000-button" id="new-folder">New Folder</button><button class="pi2000-button" id="new-profile">New Connection</button><button class="pi2000-button" id="item-open" disabled>Open</button><button class="pi2000-button" id="item-edit" disabled>Properties</button><button class="pi2000-button" id="item-delete" disabled>Delete</button></div><div class="explorer-address">Address: <span>${escape(currentPath(folder))}</span></div><div class="device-grid" tabindex="0" aria-label="Folders and SSH Profiles"></div><div class="device-context frame" hidden></div>`;
 ...
 for(const item of visible){
  const entry=document.createElement('button');entry.className='device-entry';entry.innerHTML=`<span class="pi2000-pixel-icon pi2000-icon-${item.kind==='folder'?'folder':'computer'}" aria-hidden="true"><i></i></span><span>${escape(item.name)}</span><small>${item.kind==='folder'?'Folder':escape(item.username+'@'+item.host)}</small>`;
...
function startTerminal(profile,password,terminalId=null,detached=false){
 ...
 win.body.innerHTML='<div class="terminal-toolbar"><button class="pi2000-button reconnect" disabled>Reconnect</button><span>The job continues when you disconnect or log off.</span></div><div class="host-confirm" hidden></div><div class="terminal-surface"></div>';
...
shell.actions.status=async()=>{
 ...
 async function refreshStatus(){
  try{
   const [health,runtime,versions]=await Promise.all([api('/health'),api('/runtime'),api('/version')]);
   if(disposed)return;
   ...
   const components=Object.entries(versions.components||{}).map(([name,c])=>'<tr><td>'+escape({web:'Web API',sessions:'SSH / Browser',arduino:'Arduino'}[name]||name)+'</td><td>'+escape(c.loaded?.revision||'Unknown')+'</td><td>'+escape({current:'Current',pending:'Update pending',draining:'Waiting for jobs',unknown:'Running version unavailable',unavailable:'Unavailable'}[c.state]||c.state)+'</td></tr>').join('');
   win.body.querySelector('.system-status').innerHTML=`<p>Web Server: <b>OK</b> · Session Service: <b>${health.sessions==='ok'?'OK':'Temporarily unavailable'}</b></p><table><thead><tr><th>Account</th><th>Terminals</th><th>Browser</th><th>Memory</th></tr></thead><tbody>${rows||'<tr><td colspan="4">No running sessions.</td></tr>'}</tbody></table><p>Maximum 8 terminals per account and 3 browsers in total. Each browser can use up to 1.5 GB of memory and 1.5 CPU cores.</p><p>A browser with no connected user is stopped after 24 hours. Running SSH jobs continue.</p>${account.is_owner?`<p>Backup: <b>${backup?.state==='ok'?'Restore verified '+escape(new Date(backup.created).toLocaleString('en-GB')):backup?.state==='failed'?'Last attempt failed':'No verified backup yet'}</b></p><p>Backups are stored locally on the server.</p><p>Free disk space: ${(health.disk_free_bytes/1073741824).toFixed(1)} GB</p>`:''}`;
   win.body.querySelector('.system-status').insertAdjacentHTML('beforeend','<h3>Installed and running services</h3><p>Installed: '+escape(versions.version)+' · '+escape(versions.revision)+' · Build '+escape(versions.build)+'</p><table><thead><tr><th>Service</th><th>Running revision</th><th>Status</th></tr></thead><tbody>'+components+'</tbody></table><p>Service activation is performed by the OS administrator after jobs finish.</p>');
...
window.Pi2000Apps.register({type:'explorer-window',restore:async entry=>{folder=entry.folder;await shell.actions.devices();return explorer;}});
window.Pi2000Apps.register({type:'browser-window',singleton:true,restore:()=>{shell.actions.browser();return browserWindow;}});
window.Pi2000Apps.register({type:'users-window',singleton:true,allowed:user=>user?.role==='admin',restore:async()=>{await shell.actions.users();return usersWindow;}});
window.Pi2000Apps.register({type:'terminal-window',restore:async(entry,{remaining})=>{
 ...
 const b=document.createElement('button');b.className='pi2000-button';b.textContent='My Devices';b.onclick=()=>shell.actions.devices();win.body.append(b);return win;
}});
window.Pi2000Apps.register({type:'status-window',singleton:true,restore:async()=>{await shell.actions.status();return statusWindow;}});
window.addEventListener('resize',()=>windows.forEach(win=>{keepWindowVisible(win.element);win.onresize?.();}));
window.Pi2000Desktop=Object.freeze({api,makeWindow,saveWorkspace,getUser:()=>account,scheduleWorkspace,listWindows:()=>[...windows],openConnection:profile=>connectDialog(profile),resumeTerminal:term=>startTerminal(term.profile,null,term.id)});
