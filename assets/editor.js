(() => {
 ...
 function addTab(name,text='',id=null,version=null,fileParent=parent,remote=null){
  ...
  session.on('change',()=>{session.clearAnnotations();if(win)win.body.querySelector('.editor-diagnostics').hidden=true;scheduleDraft(tab);renderTabs();status();});session.selection.on('changeCursor',status);select(tab);return tab;
 }
 ...
 function open(){
  ...
  win.body.innerHTML='<div class="editor-toolbar explorer-toolbar"></div><div class="editor-location"></div><div class="editor-layout"><aside class="editor-tree" aria-label="Files and Folders"></aside><div class="editor-main"><div class="editor-tabs"></div><div class="editor-code" aria-label="Code"></div><div class="editor-diagnostics" role="status" hidden></div></div></div>';
