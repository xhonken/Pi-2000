...
(() => {
 'use strict';
 const d=Pi2000Desktop,s=Pi2000Shell,esc=Pi2000Development.esc;let current=null;
 ...
 function open(saved={}){
  ...
  w.body.innerHTML=`<div class="tool-toolbar iptv-toolbar"></div><div class="iptv-error" role="alert" hidden></div>
   ...
   <div class="iptv-tabs" role="tablist" aria-label="Media library">${[['live','Live TV'],['movie','Movies'],['series','Series'],['favorites','Favorites'],['recent','Continue Watching'],['all','All Media']].map(([v,n])=>`<button class="pi2000-button" role="tab" data-kind="${v}" aria-selected="false">${n}</button>`).join('')}</div>
   <div class="iptv-main"><section class="iptv-library" aria-label="Channel library"><div class="iptv-filters"><input type="search" aria-label="Search IPTV" placeholder="Search channels, films or episodes…" maxlength="200"><label>Group<select aria-label="IPTV group"><option value="">All groups</option></select></label><label>Country<select aria-label="IPTV country"><option value="">All countries</option></select></label></div><div class="iptv-breadcrumb" hidden><button class="pi2000-button" type="button">Back to Series</button><span></span></div><div class="iptv-list" tabindex="0" role="listbox" aria-label="Channels and media"></div><div class="iptv-pages"><button class="pi2000-button" data-page="previous">Previous</button><span></span><button class="pi2000-button" data-page="next">Next</button></div></section>
   <section class="iptv-viewer" aria-label="Media player"><div class="iptv-screen"><video controls playsinline preload="none" aria-label="IPTV video"></video><div class="iptv-empty"><strong>Pi-IPTV</strong><span>Add a playlist, then double-click a channel or film.</span></div></div><div class="iptv-now" aria-live="polite">Nothing playing</div><div class="iptv-playback"><label>Playback<select aria-label="Playback mode"><option value="original">Original quality</option><option value="audio">Compatible audio (AAC)</option><option value="compatible">Compatible video + audio (720p)</option></select></label><label>Picture<select aria-label="Picture fit"><option value="contain">Fit</option><option value="cover">Fill</option><option value="fill">Stretch</option></select></label><label>Quality<select aria-label="Stream quality"><option value="-1">Automatic</option></select></label><label>Audio<select aria-label="Audio track"><option>Default</option></select></label><label>Subtitles<select aria-label="Subtitle track"><option value="-1">Off</option></select></label></div><section class="iptv-guide" aria-label="Programme guide"><div class="iptv-guide-heading"><strong>Programme Guide / TV Archive</strong><button class="pi2000-button" type="button">Load Guide</button></div><div class="iptv-programmes">Select a live channel to see its programme guide.</div></section></section></div>`;
  ...
  function command(label,fn){const b=document.createElement('button');b.className='pi2000-button';b.type='button';b.textContent=label;b.onclick=()=>run(fn);$('.iptv-toolbar').append(b);return b;}
  ...
  function form(title,html,submit,fn){
   s.show(title,'<form class="iptv-form">'+html+'<p class="iptv-form-message" role="alert"></p><div class="actions"><button class="pi2000-button" type="submit">'+submit+'</button><button class="pi2000-button" type="button" data-cancel>Cancel</button></div></form>');
  ...
  const accountChanged=()=>{if(d.getUser()?.id!==owner)w.close(true);};window.addEventListener('pi2000-user',accountChanged);
  ...
  w.onclose=()=>{stop(false,true);const form=document.querySelector('#window .iptv-form');if(form){form.reset();form.replaceChildren();document.querySelector('#window').close();}closed=true;clearTimeout(searchTimer);for(const c of controllers)c.abort();window.removeEventListener('pi2000-user',accountChanged);window.removeEventListener('pagehide',leaving);current=null;};
 ...
 s.actions.iptv=()=>open();Pi2000Apps.register({type:'iptv-window',singleton:true,restore:entry=>open(entry?.state)});
})();
