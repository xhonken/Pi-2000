...
function render() {
 ...
 category('Development and Drawing',[button('Pi++','editor'),button('Pi-DB Manager','database'),button('Pi-API','apitester'),button('Pi-Git Projects','git'),button('Pi-Arduino','arduino'),button('Dimension Drawing','cad'),button('Display Studio','displaystudio')],'editor');
...
const actions = {
 ...
 async about() {
  ...
  try {const response=await fetch('/api/version',{cache:'no-store'});if(!response.ok)throw Error('Could not read the installed version. Sign in and try again.');const data=await response.json();if($('#window-title').textContent!=='About Pi-2000')return;
   $('#window-content').innerHTML='<p><b>Pi-2000</b></p><p>A Windows 2000-inspired web desktop.</p><dl><dt>Installed version</dt><dd data-version></dd><dt>Build ID</dt><dd data-build></dd><dt>Source revision</dt><dd data-revision></dd></dl><p>Running SSH and Browser sessions survive a viewer disconnect. Restarting the Pi ends those processes. Database transactions are not restored after disconnect.</p>';
