...
(() => {
 ...
 async function derive(password,salt,signal){
  if(signal?.aborted)throw Error('Vault locked.');
  return new Promise((resolve,reject)=>{
   ...
   const abort=()=>finish(Error('Vault locked.'));signal?.addEventListener('abort',abort,{once:true});timer=setTimeout(()=>finish(Error('Password derivation timed out.')),60000);
 ...
 async function unlockRaw(v,slot,password,signal){validate(v);try{const k=await wrapping(password,un64(v[slot].salt),signal);return await decrypt(k,v[slot].wrapped,aad(v,'key-'+slot));}catch(error){if(signal?.aborted)throw Error('Vault locked.');throw Error('Incorrect password '+slot.toUpperCase()+' or damaged Vault.');}}
 ...
 function validIndex(v,index){exact(index,['items']);if(!Array.isArray(index.items)||index.items.length>500)throw Error('Invalid Vault list.');const seen=new Set();for(const item of index.items){exact(item,['id','title','category','created','updated']);if(!hex.test(item.id)||seen.has(item.id)||!Object.hasOwn(v.entries,item.id)||typeof item.title!=='string'||!item.title.trim()||item.title.length>200||!['Password','API key','Text','Link'].includes(item.category)||![item.created,item.updated].every(t=>Number.isSafeInteger(t)&&t>=0&&t<=8640000000000000))throw Error('Invalid Vault list.');seen.add(item.id);}if(seen.size!==Object.keys(v.entries).length)throw Error('Vault list does not match its encrypted entries.');return index;}
